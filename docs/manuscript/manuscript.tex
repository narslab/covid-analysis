...
We use transportation data provided by Apple\footnote{\url{https://www.apple.com/covid19/mobility}} along with Community
Mobility Reports compiled by
Google\footnote{\url{https://www.google.com/covid19/mobility/}}.\marginnote{\scriptsize\rd Please read this paragraph
  carefully and revise. This needs some overhauling. For instance, we are no longer using Apple.}[-3cm]
The new confirmed cases global data set is provided by the Johns Hopkins University COVID-19 Data
initiative\footnote{\url{https://github.com/CSSEGISandData/COVID-19}}.\marginnote{\scriptsize\rd There should be a
  citation in the bibliography for the JHU dataset. Let me know if you have any issues locating it.}
\st{Apple's data set focuses on transportation types
(walking, driving, transit)} whereas Google tracks movement trends by region and segments them into categories such as
residential, workplace and transit. Our integrated data set captures changes in requests for directions from January 22
to \hl{July 14} as well as aggregated location data for a variety of categorized places. The data shows us the change in
visitors from February 15 to July 12 compared to a baseline of days (January 3 - February 6, 2020).
\marginnote{\scriptsize\rd All the dates need to be updated. Again, this section needs work. }
\st{By tracking user
requests of their Maps application, Apple contributes a snapshot of incidental trips as opposed to actual foot traffic
which is the approach taken by Google for their Community Mobility Reports.}  The sources are summarized in
\autoref{tab:sources} and the variables are described in \autoref{tab:variables}.  The integrated dataset (along with
the code and plots generated for this paper) is publicly available on
Github.\footnote{\url{https://github.com/narslab/covid-analysis/tree/master/mobility}}
...
\marginnote{\scriptsize{\rd This is a good start. However, you need to rewrite it as a paragraph. Remove the overly
    technical details, such as ``melt'', ``ISO'', ``MultiIndex'', etc. Try to explain what you did in a way that is most
  accessible to the lay reader here.}}
  ...
  \hl{You also want to highlight here that the outcome of this effort is an integrated dataset that is publicly
    available. Put the github link in a footnote.}
