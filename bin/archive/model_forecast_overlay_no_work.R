...
generate_overlay_no_work <- function(bgvar_model, bgvar_forecast, var, save=FALSE) {
  ...
  predicted_observed_overlay <- ggplot(merged_data, aes(Day)) + 
    ...
    geom_line(aes(y=Observed, color = "Observed", linetype = "Observed"), size = 0.25) + # modified color, size, and linetype
  ...
  if(save) {
    ...
    print(fname)
    ggsave(filename = fname, plot = predicted_observed_overlay, width = 20, height=10, device = "png")  
  }
